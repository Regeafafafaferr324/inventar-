function vRPin.buyItem(idname, amount)
    ...
    if user_id ~= nil then
        ...
        if shop ~= nil then
            ...
            vRPclient.notify(player,{"Error. Close and open the shop again."})
        end
    end
end
...
function vRPin.openShop(name, pos)
    local user_id = vRP.getUserId({source})
    if user_id then
        if Config.Shops[name].permission and vRP.getUserFaction({user_id}) == Config.Shops[name].permission  or Config.Shops[name].permission == nil then
