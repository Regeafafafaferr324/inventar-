...
function loadTargetInventory(player, user_id, target, target_id)
    ...
    local items, weight, maxWeight = vRPin.getAllInventoryItems(target)
    INclient.openInventory(player, {"CASH: $"..vRP.formatMoney{vRP.getMoney{target_id}}})
...
exports('loadTargetInventory', loadTargetInventory);
...
function vRPin.putIntoPlayer(idname, amount)
    ...
    if target_id ~= nil then
        ...
        if new_weight <= vRP.getInventoryMaxWeight({target_id}) then
            ...
            vRPclient.notify(player,{"Target inventory is full."})
        end
